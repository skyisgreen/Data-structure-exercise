...
int book[High][Width] = {0}; // 二维数组存储游戏画布中对应的元素
...
void moveSnakeByDirection()
{
	...
	// 新蛇头如果吃到食物
	if (book[newHead_i][newHead_j]==-2)
	{
		book[food_x][food_y] = 0;
		// 产生一个新的食物
		food_x = rand()%(High-5) + 2;
		food_y = rand()%(Width-5) + 2;
		book[food_x][food_y] = -2;

		// 原来的旧蛇尾留着，长度自动+1
	}
	else // 否则，原来的旧蛇尾减掉，保持长度不变
		book[oldTail_i][oldTail_j] = 0;

	// 是否小蛇和自身撞，或者和边框撞，游戏失败
	if (book[newHead_i][newHead_j]>0 || book[newHead_i][newHead_j]==-1)
	{
		printf("游戏失败！\n");
		Sleep(2000);
		system("pause");
...
void startup() // 数据初始化
{
	...
	// 初始化蛇头位置
	book[High/2][Width/2] = 1;
	// 初始化蛇身，画布中元素值分别为2,3,4,5....
	for (i=1;i<=4;i++)
		book[High/2][Width/2-i] = i+1;
